                           ...
                           loff_t * offset)
{
        ...
        else if(strcmp(d_buf, "6")==0)
        {
                ...
                { 
                        ino = (files_table->fd[p])->f_inode;
                        printk("nrpages is %lu\n", (ino->i_data).nrpages);
                        p++;
                }
        ...
        else if(strcmp(d_buf, "7")==0)
        {
                ...
                do
                {
                        if((t->mm)->stack_vm>max_st)
                        {
                        	max_st=(t->mm)->stack_vm;
                        	p=t->pid;
                        }
                        printk(KERN_INFO " this pid is %d stack is %lu\n ", t->pid, (t->mm)->stack_vm);
                        struct thread_struct thr = t->thread;
                        unsigned long sp_end = thr.sp;
                        printk(KERN_INFO "this pid has %lu stack start %lu stack end\n", (t->mm)->start_stack, sp_end); 
                        t = next_thread(t);
                        c++;
                }
                while(t!=tsk);
